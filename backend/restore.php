...
if (isset($_GET['inv_id'])) {
    ...
    $sql = "UPDATE ingredientsHeader SET is_archived = 0 WHERE id = ?";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("i", $inv_id);
    
    if ($stmt->execute()) {
        header("Location: ../main_inventory.php?restored=1");
    } else {
...
if (isset($_GET['user_id'])) {
    ...
    $sql = "UPDATE users SET is_archived = 0 WHERE id = ?";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("i", $user_id);
    
    if ($stmt->execute()) {
        header("Location: ../employee.php?restored=1");
    } else {
