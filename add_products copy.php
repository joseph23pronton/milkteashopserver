<?php
$screen = 'products';
ob_start();
$mysqli = include('database.php');
$query = "SELECT id, name FROM main_inventory";
$result = $mysqli->query($query);

// Initialize the options variable
$options = '';
while ($row = $result->fetch_assoc()) {
    $options .= "<option value='{$row['id']}'>{$row['name']}</option>";
}
?>
<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>Add Products</title>

    <!-- Custom fonts for this template-->
    <link href="vendor/fontawesome-free/css/all.min.css" rel="stylesheet" type="text/css">
    <link
        href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i"
        rel="stylesheet">

    <!-- Custom styles for this template-->
    <link href="css/sb-admin-2.min.css" rel="stylesheet">

</head>

<body id="page-top">

    <!-- Page Wrapper -->
    <div id="wrapper">

        <?php include "backend/nav.php" ?>

        <!-- Begin Page Content -->
        <div class="container-fluid">

            <!-- Page Heading -->
            <h1 class="h3 mb-4 text-gray-800">Add Products to all Branches</h1>
            <div class="row">
                <div class="col-lg-12">
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Add Products</h6>
                        </div>
                        <div class="card-body">
                            <form action="backend/add_products_method.php" method="post">
                                <div class="form-group">
                                    <label for="branch_city">Product Name</label>
                                    <input type="text" class="form-control" name="product_name" required>
                                </div>
                                <div class="form-group">
                                    <label for="branch_city">Product Description</label>
                                    <input type="text" class="form-control" name="product_description" required>
                                </div>
                                <div class="form-group">
                                    <label for="branch_city">Product Image</label>
                                    <input type="text" class="form-control" name="image" disabled>
                                </div>
                                <div class="form-group">
                                    <label for="branch_city">Product Price</label>
                                    <input type="text" class="form-control" name="price" required>
                                </div>
                                <div class="form-group">
                                    <label for="is_active">
                                        <input type="checkbox" id="is_active" name="is_active" value="1">
                                        Active
                                    </label>
                                </div>
                                <div id="ingredients-section">
                                    <div class="form-group">
                                        <label for="inventory_id">Ingredient</label>
                                        <select class="form-control" id="inventory_id" name="inventory_id[]" required>
                                            <!-- Populate with main_inventory data dynamically -->
                                            <?= $options; ?>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="quantity_required">Quantity Required</label>
                                        <input type="number" step="0.01" class="form-control" id="quantity_required" name="quantity_required[]" required>
                                    </div>
                                </div>
                                <button type="button" class="btn btn-secondary" id="add-ingredient">Add Another Ingredient</button>
                                <button type="submit" class="btn btn-primary">Save Product</button>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- /.container-fluid -->

    </div>
    <!-- End of Main Content -->

    </div>
    <!-- End of Content Wrapper -->

    </div>
    <!-- End of Page Wrapper -->

    <!-- Scroll to Top Button-->
    <a class="scroll-to-top rounded" href="#page-top">
        <i class="fas fa-angle-up"></i>
    </a>

    <!-- Logout Modal-->
    <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Ready to Leave?</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">Select "Logout" below if you are ready to end your current session.</div>
                <div class="modal-footer">
                    <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
                    <a class="btn btn-primary" href="login.html">Logout</a>
                </div>
            </div>
        </div>
    </div>

    <!-- Bootstrap core JavaScript-->
    <script src="vendor/jquery/jquery.min.js"></script>
    <script src="vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

    <!-- Core plugin JavaScript-->
    <script src="vendor/jquery-easing/jquery.easing.min.js"></script>

    <!-- Custom scripts for all pages-->
    <script src="js/sb-admin-2.min.js"></script>

    <script>
document.getElementById('add-ingredient').addEventListener('click', function () {
    const section = document.getElementById('ingredients-section');
    const options = `<?= $options; ?>`; // Inject the PHP options here
    const ingredientRow = `
        <div class="form-group">
            <label for="inventory_id">Ingredient</label>
            <select class="form-control" name="inventory_id[]" required>
                ${options}
            </select>
        </div>
        <div class="form-group">
            <label for="quantity_required">Quantity Required</label>
            <input type="number" step="0.01" class="form-control" name="quantity_required[]" required>
        </div>
    `;
    section.insertAdjacentHTML('beforeend', ingredientRow);
});
</script>

</body>

</html>